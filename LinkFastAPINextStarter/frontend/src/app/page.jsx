...
export default function Home() {
  ...
  // Must課題②:割り算用の関数追加
  //　YOKO追加
  const handleDivisionRequest = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/division/${divisionNumber}`);
      const data = await response.json();
      setDivisionResult(data.half_value.toString());
    } catch (error) {
      console.error('Error:', error);
    }
  };
...
　//　YOKO追加
  const handleTextCount = async () => {
    try {
      const response = await fetch(`http://localhost:8000/api/text_count/${textCount}`);
      const data = await response.json();
      setTextCountResult(data.text_count.toString());
    } catch (error) {
      console.error('Error:', error);
    }
  };
  ...
  return (
    <div className="p-8">
      ...
      <div className="space-y-8">
        ...
        <section>
          <h2 className="text-xl font-bold mb-4">数値を半分にするリクエストを送信</h2>
          <div className="flex gap-2">
            <input
              type="number"
          ...
          {divisionResult && (
            <p className="mt-2">FastAPIからの応答: {divisionResult}</p>
          )}
        </section>
        ...
        <section>
          ...
          {textCountResult && (
            <p className="mt-2">文字数は: {textCountResult}文字</p>
          )}
        </section>
